...
- **❗ 禁止**: 直接 `tmux send-keys` を使うと Enter が正しく送信されないことがある
- **❗ 禁止**: 直接メッセージを入力する（例: ❗ 侍1、元気でござる）

### 報告の流れ
- **実働部隊→将軍**: 報告ファイル更新 + `./scripts/notify.sh` で通知（**必須**）
- **将軍→実働部隊**: YAML + `./scripts/notify.sh` で起こす
- 重要: notify.sh なしでは完了通知が届かず、システムが停止する

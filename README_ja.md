...
```
      あなた（上様）
           ...
           ▼ 命令を出す
    ┌─────────────┐
    │   SHOGUN    │  ← 命令を受け取り、タスク管理・分配
    └──────┬──────┘
           │ YAMLファイル + tmux
           │
  ┌──┬──┬──┴──┬──┬──┐
  │侍│侍│侍 │足│足│忍│  ← 6体のワーカーが並列実行
  │1 │2 │3  │1 │2 │者│
  └──┴──┴───┴──┴──┴─┘
```
...
| `attach.sh` | 起動中のtmuxセッションに接続 | 毎日（start.sh の後） |
...
| エージェント | 役割 | 数 |
|-------------|------|-----|
| 🏯 将軍（Shogun） | 総大将 - 命令を受け、タスク管理・分配 | 1 |
| ⚔️ 侍（Samurai） | 中核実装 - 機能開発・設計 | 3 |
| 👤 足軽（Ashigaru） | 補助作業 - 単純タスク実行 | 2 |
| 🥷 忍者（Ninja） | 緊急対応 - 高難度・機密タスク | 1 |

tmuxセッションが作成されます：
- `shogun` - 将軍 + 実働部隊（7ペイン統合）
...
将軍は：
1. タスクを分解し、YAMLファイルに書き込む
2. 実働部隊（侍・足軽・忍者）に通知
3. 即座にあなたに制御を返す（待つ必要なし！）

その間、実働部隊がバックグラウンドで並列実行します。
...
| エージェント | モデル | 思考モード | 理由 |
|-------------|--------|----------|------|
| 将軍 | Opus | 無効 | タスク管理・分配に深い推論は不要 |
| 侍 | Sonnet | 有効 | 実装作業にはフル機能が必要 |
| 足軽 | Haiku | 有効 | 単純作業を高速・低コストで |
| 忍者 | Opus | 無効 | 緊急・高難度タスクに対応 |

将軍・忍者は `MAX_THINKING_TOKENS=0` で拡張思考を無効化し、レイテンシとコストを削減。
...
2. **スケーラビリティ**: 実働部隊を増やしても構造が崩れない
3. **障害分離**: 1体のエージェントが失敗しても他に影響しない
...
4. **競合回避**: 各エージェントに専用ファイルを割り当て

### なぜ dashboard.md は将軍のみが更新するのか

1. **単一更新者**: 競合を防ぐため、更新責任者を1人に限定
2. **情報集約**: 将軍は全実働部隊の報告を受ける立場なので全体像を把握
3. **整合性**: タスク管理と状況報告を同一エージェントが担当
...
実行される処理:
1. 将軍がタスクを分解・割り当て:
   - 侍1: GitHub Copilotを調査
   - 侍2: Cursorを調査
   - 侍3: Claude Codeを調査
   - 足軽1: Codeiumを調査
   - 足軽2: Amazon CodeWhispererを調査
2. 5体が同時に調査
...
├── attach.sh                # 起動中のセッションに接続
│  └────────────────────────────────────────────────────────────┘
│
├── instructions/             # エージェント指示書
│   ├── 1_shogun.md           # 将軍の指示書
│   ├── 3_samurai.md          # 侍の指示書
│   ├── 4_ashigaru.md         # 足軽の指示書
│   └── 5_ninja.md            # 忍者の指示書
│
├── config/

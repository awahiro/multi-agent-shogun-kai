...
# ダメな例2: &&で繋ぐ
tmux send-keys -t multiagent:0.2 'メッセージ' && tmux send-keys -t multiagent:0.2 Enter
```
...
**【1回目】** メッセージを送る：
```bash
tmux send-keys -t multiagent:0.{N} 'queue/tasks/{agent}{N}.yaml に任務がある。確認して実行せよ。'
```
